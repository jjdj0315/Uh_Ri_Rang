...
| 기능 | 입력 → 동작 → 결과 |
|------|-------------------|
| AI 검색 | 자연어 입력 + 검색 클릭 → POST /api/match (query + teams) → 서버에서 Gemini 호출 → 추천 팀 + 근거 표시 |
...
**Phase 2 — 확장 기능**
1. Track 1: AI 팀매칭 검색형 (/api/match + Gemini + UI)
2. 훈수 피드 (더미 데이터 데모) — 시간 남으면
3. Track 2: AI 대화형 — 시간 남으면
...
| 순위 | 항목 | 배점 연관 |
|------|------|----------|
| 1 | Phase 1 기본 구현 | 기본 구현 30점 |
| 2 | Track 1 AI 검색형 | 확장 30점 핵심 |
| 3 | 빈 상태/에러 처리 | 완성도 25점 (안정성) |
| 4 | UX 개선 | 완성도 25점 (사용성) |
| 5 | 훈수 피드 | 확장 30점 보강 |
| 6 | Track 2 대화형 | 시간 남으면 |
| 7 | 문서/PDF | 문서 15점 (병행 작성) |

### 13.3 일정

```
Day 1       프로젝트 초기화 + 공통 레이아웃 + 데이터 초기화
Day 1~2     해커톤 목록 + 상세 (8개 탭)
Day 2~3     팀원 모집 + 랭킹
Day 3       빈 상태 UI + 입력 검증 + Vercel 배포
            ── 기본 구현 완료 ──
Day 4       Track 1 AI 팀매칭
Day 5       훈수 피드 + UX 개선
Day 6       Track 2 (시간 남으면) + 최종 점검
...
### 팀원 모집 (`/camp`)
- 페이지 제목 + 설명 문구
- AI 검색창 (입력 필드 + 검색 버튼) + 추천 결과 영역
- 해커톤 필터 드롭다운
- 팀 카드 그리드 (팀명, 모집 상태, 인원, 분야, 소개, 연락하기)
- 팀 만들기 버튼 → 폼 (팀이름, 연락처, 모집 분야, 인원)
- 빈 상태 UI
...
1. [ ] Design 문서 작성 (`daker-platform.design.md`)
2. [ ] 프로젝트 초기화 및 구현 시작
3. [ ] Vercel 배포
